...
exports.dummy = exports.onHomeResponse = exports.onIslandExplorePrompt = exports.onIslandFoundPrompt = exports.troySacrificePrompt = void 0;
const openai_1 = __importDefault(require("openai"));
const SampleGeneration_1 = require("./SampleGeneration");
const InputPromptGeneration_1 = require("./InputPromptGeneration");
const SampleGenerationHome_1 = require("./SampleGenerationHome");
...
const basePromptExploring = `You are esentially the dungeon master for this player that returns only a json object as described below. 
        ...
        Example:
            Input:
                ${SampleGeneration_1.sampleInput}
            
            Output:
                ${SampleGeneration_1.sampleOutput}
...
const troySacrificePrompt = async (userInput) => {
...
exports.troySacrificePrompt = troySacrificePrompt;
//import examples and add them 
const onIslandFoundPrompt = async (inputs, luck) => {
    ...
    const prevMessageList = inputs.filter((v, idx) => idx !== inputs.length - 1);
    console.log("got input list");
    const prevMessages = "prev messages:" + (prevMessageList.length ? prevMessageList.join("\n") : "none");
    ...
    if (txt.includes("explore"))
        return "explore";
    if (txt.includes("travel"))
        return "travel";
    return "explore";
};
exports.onIslandFoundPrompt = onIslandFoundPrompt;
const onIslandExplorePrompt = async (othersOpinions, currentScores, node, recentChatHistory, infoToPass, luck) => {
    const thisInput = (0, InputPromptGeneration_1.generateInput)(othersOpinions, currentScores, node, recentChatHistory, infoToPass, luck);
...
exports.onIslandExplorePrompt = onIslandExplorePrompt;
...
Yor task is to return the following:
    ...
    Example:
    Input:
        ${SampleGenerationHome_1.sampleInputHome}
    
    Output:
        ${SampleGenerationHome_1.sampleOutputHome}
       
Remeber, your entire response should be a json string that can be parsed with JSON.parse in js. Make sure you return a dictionary like above that is json parseable. Remember, you will receive a message responding to input, that is the ONLY one that should affect any changes. Any gold found earlier in the messages, crew that died, etc. should only be considered if its a consequence of the message responding to user input.`;
const onHomeResponse = async (othersOpinions, currentScores, node, recentChatHistory, infoToPass, luck, numSuitors) => {
    const thisInput = (0, InputPromptGeneration_1.generateHomeInput)(othersOpinions, currentScores, node, recentChatHistory, infoToPass, luck, numSuitors);
    const completion = await openai.chat.completions.create({
        model: model_version,
...
exports.onHomeResponse = onHomeResponse;
const dummy = async () => {
